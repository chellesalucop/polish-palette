...
def booking_create_view(request):
    ...
    # Get active appointments for booked slots data
...
def artist_approve_reject_view(request):
    ...
    if appointment.status not in ['Waiting', 'Rescheduling']:
        return JsonResponse({'success': False, 'message': 'Only Waiting or Rescheduling appointments can be approved/rejected.'}, status=400)
    ...
    if action == 'approve_request':
        ...
        log_booking_activity(
            ...
            artist=artist,
            request=request,
            metadata={'previous_status': 'Waiting', 'new_status': 'Rejected'}
        )
...
def client_history_view(request):
    """Client booking history — shows all past appointments with status, service, and artist details."""
    all_appointments = Appointment.objects.filter(client=request.user).select_related('service', 'artist__user')

    status_filter = request.GET.get('status', 'all')
    if status_filter == 'completed':
        history = all_appointments.filter(status='Finished')
    elif status_filter == 'cancelled':
        history = all_appointments.filter(status__in=['Cancelled', 'Rejected'])
    elif status_filter == 'upcoming':
        history = all_appointments.filter(status__in=['Waiting', 'Approved', 'On-going', 'Rescheduling'])
    else:
        history = all_appointments
